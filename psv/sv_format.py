...
def format_file(self):
  ...
  self.format_module(self.get_module(only_name=False))
...
def fm_parameters(self, para):
  ...
  while _plen != 0:
    ...
    if not match:
      ...
      print("ERROR [fm_parameters] Error 0")
    ...
    if not _type in sv_keywords.sv_keywords:
      ...
      print("ERROR [fm_parameters] Parameter type (%s) is not valid" % _type)
      _para = skip_row(_para)
      _plen = len(_para)
      continue
    ...
    if not row_has_equal_sign:
      print("ERROR [fm_parameters] Parameter (%s) has no assign operator (=)" % _type)
      _para = skip_row(_para)
      _plen = len(_para)
      continue
